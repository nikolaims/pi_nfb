...
subdelay = 0
rls = DelayedRLSPredictor(n_channels=data_filt.shape[1], delay=delay+subdelay, target_channel=15, M=10, lambda_=0.99999, mu=1)
for k in range(len(x) - delay - subdelay):
    x_pred[k + delay] = rls.predict(data_filt[delay + subdelay:][k])

plt.plot(x_pred)
plt.legend(['filt', 'filtfilt', 'pred'])
plt.ylim(-100, 100)
plt.show()
